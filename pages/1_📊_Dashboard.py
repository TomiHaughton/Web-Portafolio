...
def obtener_aportaciones_retiros(user_id, precio_dolar):
    try:
        conexion = conectar_db()
        df = pd.read_sql_query("SELECT tipo, monto, moneda FROM finanzas_personales WHERE categoria = 'Inversiones' AND user_id = ?", conexion, params=(user_id,))
        conexion.close()
...
def obtener_dolar_argentina():
    ...
    try:
        ...
        try:
            hist = yf.Ticker("ARS=X").history(period="1d")
            if not hist.empty and hist['Close'].iloc[-1] > 100:
                precio, fuente = hist['Close'].iloc[-1], "Yahoo"
        except: pass
...
def calcular_efectivo_actual(user_id):
    ...
    try:
        conexion = conectar_db()
        df_finanzas = pd.read_sql_query("SELECT tipo, categoria, monto, moneda FROM finanzas_personales WHERE user_id = ?", conexion, params=(user_id,))
        df_ops = pd.read_sql_query("SELECT tipo, cantidad, precio, moneda FROM operaciones WHERE user_id = ?", conexion, params=(user_id,))
        conexion.close()
        if 'moneda' not in df_finanzas.columns: df_finanzas['moneda'] = 'ARS'
        if 'moneda' not in df_ops.columns: df_ops['moneda'] = 'USD'
        df_finanzas['moneda'] = df_finanzas['moneda'].fillna('ARS')
        ...
        for _, row in df_ops.iterrows():
            monto_op = row['cantidad'] * row['precio']
            moneda = row['moneda']
            if row['tipo'] == 'Compra':
                if moneda == 'USD': saldo_usd -= monto_op
                else: saldo_ars -= monto_op
            elif row['tipo'] == 'Venta':
                if moneda == 'USD': saldo_usd += monto_op
                else: saldo_ars += monto_op
...
# 1. OBTENER DÓLAR Y GUARDARLO EN SESIÓN
precio_dolar_hoy, fuente_dolar = obtener_dolar_argentina()
st.session_state['precio_dolar_compartido'] = precio_dolar_hoy # <--- CLAVE PARA SINCRONIZAR
...
if not operaciones_df.empty or total_aportado > 0 or saldo_efectivo_usd != 0:
    ...
    st.subheader("Componentes del Patrimonio")
    r2 = st.columns(4)
